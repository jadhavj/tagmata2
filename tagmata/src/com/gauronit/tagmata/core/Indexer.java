...
//    private Map<String, IndexWriter> indexWriters = new LinkedHashMap<String, IndexWriter>();
    ...
    public ArrayList getIndexNames() {
        try {
            IndexReader ir = IndexReader.open(FSDirectory.open(new File(indexDir + File.separator + MAIN_INDEX), new SimpleFSLockFactory(indexDir + File.separator + MAIN_INDEX)));
            ...
            for (Map.Entry index : indexSearchers.entrySet()) {
                ...
                Query q = new QueryParser(Version.LUCENE_35, "indexName", new StandardAnalyzer(Version.LUCENE_35)).parse(indexName);
                TopScoreDocCollector collector = TopScoreDocCollector.create(1, true);
                mainIndexSearcher.search(q, collector);
                ScoreDoc[] hits = collector.topDocs().scoreDocs;

                indexNames.add(new String[]{indexName, mainIndexSearcher.doc(hits[0].doc).get("displayName")});
            }
            ir.close();
    ...
    private synchronized void loadIndexes() throws Exception {
        try {
            ...
            IndexReader ir = null;
            try {
                ir = IndexReader.open(FSDirectory.open(new File(indexDir + File.separator + MAIN_INDEX), new SimpleFSLockFactory(indexDir + File.separator + MAIN_INDEX)));
                mainIndexWriter = new IndexWriter(FSDirectory.open(new File(indexDir + File.separator + MAIN_INDEX)), new IndexWriterConfig(Version.LUCENE_35, new StandardAnalyzer(Version.LUCENE_35)));
            } catch (Exception ex) {
                try {
                    IndexWriter iw = new IndexWriter(FSDirectory.open(new File(indexDir + File.separator + MAIN_INDEX)), new IndexWriterConfig(Version.LUCENE_35, new StandardAnalyzer(Version.LUCENE_35)));
                    iw.close();
                    mainIndexWriter = new IndexWriter(FSDirectory.open(new File(indexDir + File.separator + MAIN_INDEX)), new IndexWriterConfig(Version.LUCENE_35, new StandardAnalyzer(Version.LUCENE_35)));
                    ir = IndexReader.open(FSDirectory.open(new File(indexDir + File.separator + MAIN_INDEX), new SimpleFSLockFactory(indexDir + File.separator + MAIN_INDEX)));
                } catch (Exception e) {
            ...
            for (ScoreDoc hit : hits) {
                ...
                IndexReader reader = IndexReader.open(FSDirectory.open(new File(indexDir + File.separator + indexName), new SimpleFSLockFactory(indexDir + File.separator + indexName)));
                IndexSearcher searcher = new IndexSearcher(reader);
                indexSearchers.put(indexName, searcher);
//                IndexWriter writer = new IndexWriter(FSDirectory.open(new File(indexDir + File.separator + indexName)), new IndexWriterConfig(Version.LUCENE_35, new StandardAnalyzer(Version.LUCENE_35)));
//                indexWriters.put(indexName, writer);
    ...
    public String createIndex(String indexDisplayName) {
        try {
            ...
            IndexWriter iw = new IndexWriter(FSDirectory.open(new File(indexDir + File.separator + indexName)), new IndexWriterConfig(Version.LUCENE_35, new StandardAnalyzer(Version.LUCENE_35)));
            iw.prepareCommit();
            iw.commit();
            iw.close();
//            indexWriters.put(indexName, iw);
            IndexReader ir = IndexReader.open(FSDirectory.open(new File(indexDir + File.separator + indexName), new SimpleFSLockFactory(indexDir + File.separator + indexName)));
    ...
    public void renameIndex(String indexName, String indexDisplayName) {
        try {
            ...
            mainIndexWriter.updateDocument(new Term("indexName", indexName), doc, new StandardAnalyzer(Version.LUCENE_35));
    ...
    public void deleteCards(ArrayList<CardSnapshot> cardSnaps) {
        try {
            for (CardSnapshot cardSnap : cardSnaps) {
                IndexWriter writer = new IndexWriter(FSDirectory.open(new File(indexDir + File.separator + cardSnap.getIndexName())), new IndexWriterConfig(Version.LUCENE_35, new StandardAnalyzer(Version.LUCENE_35)));
    ...
    public void saveBookmark(String id, String title, String tags, String text, String indexName) {
        try {
            mainIndexWriter.close();
            mainIndexWriter = new IndexWriter(FSDirectory.open(new File(indexDir + File.separator + MAIN_INDEX)), new IndexWriterConfig(Version.LUCENE_35, new StandardAnalyzer(Version.LUCENE_35)));
            IndexReader ir = IndexReader.open(FSDirectory.open(new File(indexDir + File.separator + MAIN_INDEX), new SimpleFSLockFactory(indexDir + File.separator + MAIN_INDEX)));
            ...
            if (hits != null && hits.length == 0) {
                ...
                mainIndexWriter.updateDocument(new Term("id"), doc, new StandardAnalyzer(Version.LUCENE_35));
                mainIndexWriter.prepareCommit();
                mainIndexWriter.commit();
                mainIndexWriter.close();
            }
    ...
    public void deleteBookmark(String id, String indexName) {
        try {
            mainIndexWriter = new IndexWriter(FSDirectory.open(new File(indexDir + File.separator + MAIN_INDEX)), new IndexWriterConfig(Version.LUCENE_35, new StandardAnalyzer(Version.LUCENE_35)));
            mainIndexWriter.deleteDocuments(new Term("id", id));
            mainIndexWriter.prepareCommit();
            mainIndexWriter.commit();
            mainIndexWriter.close();
        } catch (Exception ex) {
    ...
    public ArrayList<CardSnapshot> getBookmarks() {
        ...
        try {
            ...
            Query q = new WildcardQuery(new Term("id", "*"));
            TopScoreDocCollector collector = TopScoreDocCollector.create(10000, false);
            mainIndexSearcher.search(q, collector);
            ScoreDoc[] hits = collector.topDocs().scoreDocs;
            for (ScoreDoc hit : hits) {
                Document doc = mainIndexSearcher.doc(hit.doc);
                IndexReader reader = IndexReader.open(FSDirectory.open(new File(indexDir + File.separator + doc.get("indexName")), new SimpleFSLockFactory(indexDir + File.separator + doc.get("indexName"))));
                IndexSearcher searcher = new IndexSearcher(reader);

                q = new TermQuery(new Term("id", doc.get("id")));
    ...
    public ArrayList<CardSnapshot> search(String searchText, ArrayList<String> indexNames, boolean searchInTitle, boolean searchInTags, boolean searchInText, boolean superFuzzy) {
        ArrayList<CardSnapshot> cardSnaps = new ArrayList();
        try {
            ...
            for (IndexSearcher searcher : searchers) {
                ...
                for (ScoreDoc hit : hits) {
                    ...
                    TokenStream stream = TokenSources.getTokenStream("text", doc.get("analyzedText"), new StandardAnalyzer(Version.LUCENE_20.LUCENE_35));
