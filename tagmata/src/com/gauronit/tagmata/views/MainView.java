...
public class MainView extends JFrame {
	...
	public MainView() {
		...
		TaskMonitor taskMonitor = new TaskMonitor(Main.getApplication()
				.getContext());
		taskMonitor
		...
		trayIcon = new TrayIcon(resourceMap.getImageIcon("tagmataIcon")
				.getImage(), "Tray Demo", popup);
		trayIcon.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (e.getButton() == MouseEvent.BUTTON2) {
					popup.show(Main.getApplication().getMainFrame(), e.getX(),
							e.getY());
				}
			}
	...
	private void loadIndexes() {
		...
		DefaultTableModel tm = new IndexesTableModel(new Object[][] {},
		...
		indexesTable.getColumnModel().removeColumn(
				indexesTable.getColumnModel().getColumn(2));
	...
	private static class Indexes {
		...
		private static class Index {
			...
			public Index(String indexName, String displayName,
					boolean searchable) {
		...
		public static void loadIndexes() {
			try {
				ArrayList<String[]> indexNames = Indexer.currentInstance()
						.getIndexNames();
		...
		public static void addIndex(String displayName) {
			...
			indexes.put(indexName.toString(), new Index(indexName.toString(),
					displayName, true));
		}
				...
				public void tableChanged(TableModelEvent e) {
					...
					if (row < getRowCount()) {
						if (getValueAt(row, 1) != null
								...
								&& e.getColumn() != 0) {
							try {
								String indexDisplayName = getValueAt(row, 1)
										.toString();
								String indexName = getValueAt(row, 2)
										.toString();
								Indexer.currentInstance().renameIndex(
										indexName, indexDisplayName);
							} catch (Exception ex) {
								ex.printStackTrace();
							}
						}
	...
	private void initComponents() {
		...
		deleteQuickCard.setToolTipText("Delete selected 'Quick Card'");
		...
		tagsChkBox.setToolTipText("Search 'Tags' fields");
		titleChkBox = new javax.swing.JCheckBox();
		titleChkBox.setToolTipText("Search 'Title' fields");
		textChkBox = new javax.swing.JCheckBox();
		textChkBox.setToolTipText("Search 'Text' fields");
		jLabel5 = new javax.swing.JLabel();
		jLabel5.setToolTipText("Search Tagmata");
		newCardLbl = new javax.swing.JLabel();
		newCardLbl.setToolTipText("Save a new card");
		...
		addIndexLbl.setToolTipText("Add index");
		...
		outerSplitPane.setBorder(javax.swing.BorderFactory
		...
		org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application
				.getInstance(com.gauronit.tagmata.Main.class).getContext()
		...
		javax.swing.GroupLayout quickCardsPanelLayout = new javax.swing.GroupLayout(
				quickCardsPanel);
		quickCardsPanelLayout
				.setHorizontalGroup(quickCardsPanelLayout
						.createParallelGroup(Alignment.LEADING)
						.addGroup(
								quickCardsPanelLayout
										.createSequentialGroup()
										...
										.addContainerGap()));
		quickCardsPanelLayout
				.setVerticalGroup(quickCardsPanelLayout
						.createParallelGroup(Alignment.LEADING)
						.addGroup(
								quickCardsPanelLayout
										.createSequentialGroup()
		...
		javax.swing.GroupLayout resultsPanelLayout = new javax.swing.GroupLayout(
				resultsPanel);
		resultsPanel.setLayout(resultsPanelLayout);
		resultsPanelLayout
				.setHorizontalGroup(resultsPanelLayout
						.createParallelGroup(
		...
		resultsPanelLayout
				.linkSize(javax.swing.SwingConstants.HORIZONTAL,
						new java.awt.Component[] { tagsChkBox, textChkBox,
								titleChkBox });

		resultsPanelLayout
				.setVerticalGroup(resultsPanelLayout
						.createParallelGroup(
								javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(
								resultsPanelLayout
		...
		resultsPanelLayout.linkSize(javax.swing.SwingConstants.VERTICAL,
				new java.awt.Component[] { allChkBox, deleteCardLbl,
						newCardLbl, tagsChkBox, textChkBox, titleChkBox });
		...
		searchAllIndices.setSelected(true);
		searchAllIndices
				.setText(resourceMap.getString("searchAllIndices.text")); // NOI18N
		...
		statusPanelLayout
				.setHorizontalGroup(statusPanelLayout
						...
						.addComponent(statusPanelSeparator,
								javax.swing.GroupLayout.DEFAULT_SIZE, 1061,
								Short.MAX_VALUE)
						.addGroup(
								statusPanelLayout
										.createSequentialGroup()
	...
	private void innerSplitPaneComponentResized(
			java.awt.event.ComponentEvent evt) {// GEN-FIRST:event_innerSplitPaneComponentResized
	...
	private void addIndexLblMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_addIndexLblMouseClicked
		// TODO add your handling code here:
		Object indexDisplayName = JOptionPane.showInputDialog(Main
				.getApplication().getMainFrame(), "Name for the new index",
				"Index Creation", JOptionPane.PLAIN_MESSAGE, resourceMap
						.getIcon("newIndexIcon2"), null, null);
		if (indexDisplayName != null
				&& indexDisplayName.toString().trim().length() != 0) {
			if (indexDisplayName.toString().length() >= 100) {
				JOptionPane
						.showMessageDialog(
								Main.getApplication().getMainFrame(),
								"The index name you enter is too long. A maximum of 100 letters is permissible.",
								"Invalid Index Name", JOptionPane.ERROR_MESSAGE);
			} else {
				for (Map.Entry<String, Indexes.Index> entry : Indexes
						.getIndexes().entrySet()) {
					...
					if (index.getDisplayName().equals(indexDisplayName)) {
						JOptionPane
								.showMessageDialog(
										Main.getApplication().getMainFrame(),
										"Index with the same name already exists. Please choose a different name.",
										"Index Creation",
				...
				try {
					String indexName = Indexer.currentInstance().createIndex(
							indexDisplayName.toString());
					((DefaultTableModel) indexesTable.getModel())
							.addRow(new Object[] { true,
									indexDisplayName.toString(), indexName });
				} catch (Exception ex) {
					ex.printStackTrace();
				}
	...
	private void deleteIndexMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_deleteIndexMouseClicked
		...
		if (row != -1) {
			String indexDisplayName = indexesTable.getValueAt(row, 1)
					.toString();
			for (int i = 0; i < indexesTable.getModel().getRowCount(); i++) {
				if (indexesTable.getModel().getValueAt(i, 1).toString()
						.equals(indexDisplayName)) {
					String indexName = indexesTable.getModel().getValueAt(i, 2)
							.toString();
					if (indexDisplayName.equals("General")) {
						JOptionPane
								.showMessageDialog(
										Main.getApplication().getMainFrame(),
										"Can't delete the main index. Use it to save cards that are miscellaneous.",
										"Index Deletion",
					...
					int choice = JOptionPane
							.showConfirmDialog(
									...
									"Delete Index '"
											+ indexDisplayName
											+ "'? All the cards saved in it will be deleted and will not be recovered.",
									"Index Deletion", JOptionPane.YES_NO_OPTION);
					if (choice == 0) {
						((DefaultTableModel) indexesTable.getModel())
								.removeRow(i);
	...
	public void fillResults() {
		try {
			...
			for (int i = 0; i < indexesCount; i++) {
				boolean selected = (Boolean) indexesTable.getModel()
						.getValueAt(i, 0);
				if (selected) {
					indexNames.add(indexesTable.getModel().getValueAt(i, 2)
							.toString());
				}
			}
			...
			cardSnaps = Indexer.currentInstance().search(text, indexNames,
					searchTitle, searchTags, searchText,
					text.trim().equals("") ? true : superFuzzy.isSelected());
			for (CardSnapshot cardSnap : cardSnaps) {
				String item = "<html><b>Title </b>"
						+ StringUtil.getHTMLEntities(cardSnap.getTitle());
				item += "<br/><b>Tags</b> "
						+ StringUtil.getHTMLEntities(cardSnap.getTags());
				item += "<br/>"
						+ StringUtil.getHTMLEntities(cardSnap.getHighlights());
				item += "<br/>";
				item += "<table width='600px'><tr><td><hr/></td></tr></table></html>";
				((DefaultListModel) results.getModel()).add(results.getModel()
						.getSize(), item);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private void resultsMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_resultsMouseClicked
		...
		if (evt.getClickCount() == 2 && !evt.isConsumed()) {
			if (results.getModel().getSize() != 0) {
				CardSnapshot cardSnap = cardSnaps.get(results
						.getSelectedIndex());

				CardView card = new CardView(cardSnap, Main.getApplication()
						.getMainFrame(), true);
	...
	private void deleteCardLblMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_deleteCardLblMouseClicked
		// TODO add your handling code here:
		if (results.getModel().getSize() != 0) {
			int result = JOptionPane.showConfirmDialog(Main.getApplication()
					.getMainFrame(), "Delete selected cards?",
					"Cards Deletion", JOptionPane.OK_CANCEL_OPTION, 0,
					resourceMap.getIcon("deleteCardLbl.icon"));
			if (result == 2) {
				return;
			}
	...
	private void addBookmarkLblMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_addBookmarkLblMouseClicked
		...
		if (results.getModel().getSize() != 0
			...
			try {
				for (int i = 0; i < results.getSelectedIndices().length; i++) {

					CardSnapshot cardSnap = cardSnaps.get(results
							.getSelectedIndices()[i]);
					Indexer.currentInstance().saveBookmark(cardSnap.getId(),
							cardSnap.getIndexName());
				}
			} catch (Exception ex) {
	...
	private void deleteQuickCardMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_deleteQuickCardMouseClicked
		...
		if (quickCards.getModel().getSize() != 0
			...
			try {
				for (int i = 0; i < quickCards.getSelectedIndices().length; i++) {
					CardSnapshot cardSnap = bookmarks.get(quickCards
							.getSelectedIndices()[i]);
					Indexer.currentInstance().deleteBookmark(cardSnap.getId(),
							cardSnap.getIndexName());
				}
			} catch (Exception ex) {
				ex.printStackTrace();
	...
	private void helpMenuItemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_helpMenuItemActionPerformed
		...
		try {
			java.net.URI uri = new java.net.URI(
					"http://www.gauronit.com/tagmata.html");
			desktop.browse(uri);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			...
			for (CardSnapshot cardSnap : bookmarks) {
				...
				if (cardSnap.getText().length() > 100) {
					item += "<br/>"
							+ StringUtil.getHTMLEntities(cardSnap.getText()
									.substring(0, 100));
				} else {
					item += "<br/>"
							+ StringUtil.getHTMLEntities(cardSnap.getText());
	...
	private void allChkBoxActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (allChkBox.isSelected()) {
			...
			if (titleChkBox.isSelected() && tagsChkBox.isSelected()
					&& textChkBox.isSelected()) {
	...
	public String getSelectedIndexName() {
		...
		for (int i = 0; i < indexesTable.getModel().getRowCount(); i++) {
			if (((String) indexesTable.getModel().getValueAt(i, 1))
					.equals(indexDisplayName)) {
	...
	public String getIndexDisplayName(String indexName) {
		for (int i = 0; i < indexesTable.getModel().getRowCount(); i++) {
			if (((String) indexesTable.getModel().getValueAt(i, 2))
					.equals(indexName)) {
